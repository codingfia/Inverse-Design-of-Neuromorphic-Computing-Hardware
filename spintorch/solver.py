...
from .geom import WaveGeometryArray_Ms
...
class MMSolver(nn.Module):
    ...
    def __init__(self, geometry, dt: float, sources=[], probes=[]):
        ...
        self.Alpha = Damping(self.geom.dim, self.geom.mask)
    ...
    def run_stage(self, m, B_ext, Msat, signal):
        ...
        for sig in signal.split(1,dim=1):
            ...
            # Propagate the fields (with checkpointing to save memory)F
...
#             Msat = self.geom()
#             B_ext = self.geom.B
#         if isinstance(self.geom, WaveGeometryArray_Ms):
#             #print('here MF')
#             Msat = self.geom.Msat
#             B_ext = self.geom()
#         else:
