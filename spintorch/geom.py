...
class WaveGeometryArray(WaveGeometry):
    def __init__(self, rho, dim: tuple, d: tuple, Ms: float, B0: float, B0_theta: float,
                  r0_x: int, r0_y: int, dr: int, dm: int, z_off: int, rx: int, ry: int,
                  Ms_CoPt: float, beta: float = 100.0):
    

        super().__init__(dim, d, B0, B0_theta, Ms)
        self.B0_theta = B0_theta
        self.r0_x = r0_x
        self.r0_y = r0_y
        self.dr = dr
        self.rx = rx
        ...
        self.register_buffer("B", zeros((3,)+dim))
        self.B[0,] += self.B0*cos(self.B0_theta)
        self.B[1,] += self.B0*sin(self.B0_theta)

    def forward(self):
        # print('Ms',self.Ms)
        mu0 = 4*pi*1e-7
        nx, ny, nz = int(self.dim[0]), int(self.dim[1]), 1
        r0_x, r0_y, dr, rx, ry = self.r0_x, self.r0_y, self.dr, self.rx, self.ry
        rho_binary = binarize(self.rho)   
        m_rho = zeros((1, 3, ) + self.dim, device=self.B0.device)
        m_rho[0, 2, r0_x:r0_x+rx*dr:dr, r0_y:r0_y+ry*dr:dr] = rho_binary
...
class WaveGeometryArray_Ms(WaveGeometry):
    def __init__(self, rho, dim: tuple, d: tuple, Ms: float, B0: float, B0_theta: float,
                  r0_x: int, r0_y: int, dr: int, dm: int, z_off: int, rx: int, ry: int,
                  Ms_CoPt: float, mask: float, beta: float = 100.0):
